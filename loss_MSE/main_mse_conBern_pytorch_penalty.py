"""
main_mse_conBern_pytorch_penalty.py - self-contained
...
Per ciascun esperimento vengono prodotti:
  expN_pdf.png         : PDF confronto + barplot MSE
...
OUT = os.path.join(cartella_script, 'immagini_mse_conBern_pytorch_penalty')
...
# ---------------------------------------------------
# METODI DI OTTIMIZZAZIONE - (da methods.py, con MSE)
# ---------------------------------------------------
...
def solve_scipy_mse(N: int, x: np.ndarray, f: np.ndarray,
    ...
    """
    Minimizza MSE(W_ref + Delta) con SLSQP.
    Gradiente L2: grad_Delta = -2 * (M.T @ r) / n_pts  con r = f - M(W_ref+Delta).
    """
    M = basis_matrix(N, x)
    n_pts = len(x)
...
def solve_pytorch_mse(N: int, x: np.ndarray, f: np.ndarray,
    ...
    # Gradient descent Adam con MSELoss, nessun vincolo SO
...
def solve_pytorch_ordered_mse(N: int, x: np.ndarray, f: np.ndarray,
                              ...
                              lam: float = 200.0):
    """
    Gradient descent con penalità SO e MSELoss:
    ...
    # inizializza dai pesi L2 non vincolati
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    """
    Esegue tutti i metodi con L1 loss e registra W, Delta, MSE, tempo.
...
def plot_pdf_comparison(results: dict, save_path=None):
    ...
    ax_mse.set_ylabel('MSE'); ax_mse.set_title('MSE per metodo')
