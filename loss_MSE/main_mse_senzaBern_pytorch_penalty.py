"""
main_mse_senzaBern_pytorch_penalty.py  -  self-contained

Ottimizzazione con distanza L2 (MSE).
- Target: Funzioni analitiche esatte valutate su x (senza mappare su BP).
- Grado Target: 2N+1
- Grado Approssimante: N
- Controllo Ordine Stocastico: Eseguito rigorosamente sulle CDF.
...
OUT = os.path.join(cartella_script, 'immagini_mse_senzaBern_pytorch_penalty')
...
def solve_pytorch_ordered_mse(N: int, x: np.ndarray, f: np.ndarray,
    ...
    W_init = solve_pytorch_mse(N, x, f)
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (pen.)    MSE={m:.7f}   t={t:7.1f} ms  SO:{sat}")
...
STYLE = {
    ...
    'pytorch_upper':      dict(color='#c0392b', ls='--', lw=1.6, label='PyTorch MSE + SO (pen.)'),
}
