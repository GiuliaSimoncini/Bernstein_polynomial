"""
main_jensen_senzaBern_pytorch_penalty.py - self-contained

Ottimizzazione con distanza JSD (Jensen-Shannon Divergence).
...
OUT = os.path.join(cartella_script, 'immagini_jensen_senzaBern_pytorch_penalty')
...
def solve_scipy_jsd(N: int, x: np.ndarray, f: np.ndarray,
    ...
    # Usa approssimazione numerica del gradiente (più stabile per JSD manuale)
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    # Riepilogo
    print(f"\n  {'Metodo':<28}  {'JSD Loss':>12}  {'Tempo (ms)':>12}  SO")
    print(f"  {'-'*58}")
...
def plot_pdf_comparison(results: dict, save_path=None):
    ...
    ax_mse.set_ylabel('JSD (Divergence)'); ax_mse.set_title('Jensen-Shannon Divergence')
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*140}")
    print(f"  Tutti i 12 esperimenti (funzioni target PURE valutate con JSD) completati.")
