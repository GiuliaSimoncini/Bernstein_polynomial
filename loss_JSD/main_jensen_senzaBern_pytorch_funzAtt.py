"""
main_jesen_senzaBern_pytorch_funzAtt.py - self-contained

Ottimizzazione con distanza JSD.
...
OUT = os.path.join(cartella_script, 'immagini_jensen_senzaBern_pytorch_funzAtt')
...
def solve_scipy_jsd(N: int, x: np.ndarray, f: np.ndarray,
    ...
    def objective(Delta):
        # Usiamo direttamente la funzione JSD precedentemente calcolata
...
class _StochasticOrderedModelJSD(nn.Module):
    """
...
def pytorch_jsd_loss(output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    P = torch.clamp(target, min=1e-12)
    P = P / torch.sum(P)
    
    Q = torch.clamp(output, min=1e-12)
    Q = Q / torch.sum(Q)
    
    M_mix = 0.5 * (P + Q)
    
    # Proprietà dei logaritmi per evitare instabilità numeriche: log(A/B) = log(A) - log(B)
    kl_p = torch.sum(P * (torch.log(P) - torch.log(M_mix)))
    kl_q = torch.sum(Q * (torch.log(Q) - torch.log(M_mix)))
...
def solve_pytorch_ordered_jsd(N: int, x: np.ndarray, f: np.ndarray,
    ...
    model = _StochasticOrderedModelJSD(N, W_ref, direction=direction)
    ...
    for _ in range(epochs):
        opt.zero_grad()
        # Calcola solo il JSD. I vincoli sono matematicamente garantiti dal forward pass.
        jsd_term = loss_fn(model(M_t), f_t)
        jsd_term.backward()
        opt.step()
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    def _store(label, W, elapsed, Delta=None):
        val = jsd_loss(W, M, f)
        results[label] = dict(W=W, jsd=val, time_ms=elapsed, Delta=Delta)
        return val
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (Attiv.) upp  JSD={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # PyTorch + SO lower (Nuova Attivazione)
    t0 = time.perf_counter()
    W_pt_lo, D_pt_lo = solve_pytorch_ordered_jsd(N, x, f, W_init.copy(),
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (Attiv.) low  JSD={m:.7f}   t={t:7.1f} ms  SO:{sat}")
...
STYLE = {
    ...
    'pytorch_upper':      dict(color='#c0392b', ls='--', lw=1.6, label='PyTorch JSD + SO upp (Attivaz.)'),
    'pytorch_lower':      dict(color="#c02b93", ls='--', lw=1.6, label='PyTorch JSD + SO low (Attivaz.)')
}
...
def plot_pdf_comparison(results: dict, save_path=None):
    ...
    fig, (ax_pdf, ax_jsd) = plt.subplots(1, 2, figsize=(14, 5))
    ...
    jsds  = [results[k]['jsd']      for k in keys]
    times = [results[k]['time_ms']  for k in keys]
    cols  = [STYLE[k]['color']      for k in keys]
    lbls  = [STYLE[k]['label']      for k in keys]

    bars = ax_jsd.bar(range(len(keys)), jsds, color=cols, alpha=0.8)
    ax_jsd.set_xticks(range(len(keys)))
    ax_jsd.set_xticklabels(lbls, rotation=40, ha='right', fontsize=7)
    ax_jsd.set_ylabel('JSD'); ax_jsd.set_title('JSD per metodo')
    ax_jsd.grid(True, axis='y', alpha=0.3)
    for bar, t in zip(bars, times):
        h = bar.get_height()
        label = f'{t:.0f}ms' if t > 0.5 else '<1ms'
        ax_jsd.text(bar.get_x() + bar.get_width() / 2, h * 1.04,
...
def plot_weights(results: dict, save_path=None):
    ...
    for ax, key in zip(axes, keys):
        ...
        ax.set_title(f'{st["label"]}\nJSD={results[key]["jsd"]:.5f}', fontsize=7)
