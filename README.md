...
- **Solutore SciPy SLSQP (`scipy_upper/lower`)**
  ...
  - **Riferimento (`W_ref = W_bernsteinOp`)**: l'ordinamento stocastico viene calcolato e imposto rispetto ai pesi analitici dell'operatore di Bernstein. Questa è una scelta di robustezza teorica: mentre l'ottimizzazione pura di SciPy può introdurre lievi artefatti numerici o overfitting per abbattere l'MSE, l'operatore di Bernstein rappresenta il campionamento naturale e la baseline liscia e strutturalmente consistente della funzione bersaglio. Usarlo come riferimento garantisce che lo spostamento della massa (dominanza stocastica) avvenga rispetto a una base solida, liscia e matematicamente garantita.
