"""
main_l1_senzaBern_pytorch_funzAtt.py - self-contained

Ottimizzazione con distanza MAE.
...
OUT = os.path.join(cartella_script, 'immagini_l1_senzaBern_pytorch_funzAtt')
...
def mae(W: np.ndarray, M: np.ndarray, f: np.ndarray) -> float:
    # Mean Absolute Error (L1)
    return float(np.mean(np.abs(f - M @ W)))
...
def solve_scipy_mae(N: int, x: np.ndarray, f: np.ndarray,
    ...
    def jac_obj(Delta):
        r = f - M @ (W_ref + Delta)
        # Il gradiente del MAE usa la funzione segno (subgradiente)
        return -(M.T @ np.sign(r)) / n_pts
...
class _BernsteinModelMAE(nn.Module):
...
class _StochasticOrderedModelMAE(nn.Module):
    """
...
def solve_pytorch_mae(N: int, x: np.ndarray, f: np.ndarray,
...
def solve_pytorch_ordered_mae(N: int, x: np.ndarray, f: np.ndarray,
                              W_ref: np.ndarray,
                              direction: str = 'upper',
                              epochs: int = 4000,
                              lr: float = 0.05):
    ...
    model = _StochasticOrderedModelMAE(N, W_ref, direction=direction)
    ...
    for _ in range(epochs):
        opt.zero_grad()
        # Calcola solo il MAE. I vincoli sono matematicamente garantiti dal forward pass.
        mae_term = loss_fn(model(M_t), f_t)
        mae_term.backward()
        opt.step()
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    print(f"  Bernstein operator   MAE={m:.7f}   t={t_bern:7.1f} ms")

    # BernOp + SO upper
    t0 = time.perf_counter()
    W_bo_up, D_bo_up = solve_scipy_mae(N, x, f,
                                      W_init=W_init.copy(),
                                      direction='upper',
                                      W_ref=W_init.copy())
    ...
    print(f"  BernOp+SO upper      MAE={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # BernOp + SO lower
    t0 = time.perf_counter()
    W_bo_lo, D_bo_lo = solve_scipy_mae(N, x, f,
                                      W_init=W_init.copy(),
                                      direction='lower',
                                      W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  BernOp+SO lower      MAE={m:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    print(f"  Ing-Cane             MAE={m:.7f}   t={t:7.1f} ms")

    # Scipy MAE unconstrained
    t0 = time.perf_counter()
    W_scipy, _ = solve_scipy_mae(N, x, f, W_init=W_init.copy())
    t_scipy = (time.perf_counter() - t0) * 1e3
    m = _store('scipy', W_scipy, t_scipy)
    print(f"  Scipy MAE            MAE={m:.7f}   t={t_scipy:7.1f} ms")

    # Scipy + SO upper
    t0 = time.perf_counter()
    W_up, D_up = solve_scipy_mae(N, x, f,
                                 W_init=W_scipy.copy(),
                                 direction='upper',
                                 W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    print(f"  Scipy+SO upper       MAE={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # Scipy + SO lower
    t0 = time.perf_counter()
    W_lo, D_lo = solve_scipy_mae(N, x, f,
                                 W_init=W_scipy.copy(),
                                 direction='lower',
                                 W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    print(f"  Scipy+SO lower       MAE={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # PyTorch MAE unconstrained
    t0 = time.perf_counter()
    W_pt = solve_pytorch_mae(N, x, f, epochs=epochs_pt)
    t_pytorch = (time.perf_counter() - t0) * 1e3
    m = _store('pytorch', W_pt, t_pytorch)
    print(f"  PyTorch MAE          MAE={m:.7f}   t={t_pytorch:7.1f} ms")

    # PyTorch + SO upper (Nuova Attivazione)
    t0 = time.perf_counter()
    W_pt_up, D_pt_up = solve_pytorch_ordered_mae(N, x, f, W_init.copy(),
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (Attiv.) upp  MAE={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # PyTorch + SO lower (Nuova Attivazione)
    t0 = time.perf_counter()
    W_pt_lo, D_pt_lo = solve_pytorch_ordered_mae(N, x, f, W_init.copy(),
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (Attiv.) low  MAE={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # Riepilogo
    print(f"\n  {'Metodo':<28}  {'MAE':>12}  {'Tempo (ms)':>12}  SO")
    print(f"  {'-'*58}")
...
STYLE = {
    ...
    'scipy':              dict(color='#27ae60', ls='-',  lw=2.0, label='Scipy MAE SLSQP'),
    'scipy_upper':        dict(color='#e67e22', ls='-',  lw=1.8, label='Scipy MAE + SO upper'),
    'scipy_lower':        dict(color='#8e44ad', ls='-',  lw=1.8, label='Scipy MAE + SO lower'),
    'pytorch':            dict(color='#2980b9', ls='--', lw=1.8, label='PyTorch MAE'),
    'pytorch_upper':      dict(color='#c0392b', ls='--', lw=1.6, label='PyTorch MAE + SO upp (Attivaz.)'),
    'pytorch_lower':      dict(color="#c02b93", ls='--', lw=1.6, label='PyTorch MAE + SO low (Attivaz.)')
}
...
def plot_pdf_comparison(results: dict, save_path=None):
    ...
    ax_mse.set_ylabel('MAE (L1)'); ax_mse.set_title('MAE per metodo')
...
def plot_weights(results: dict, save_path=None):
    ...
    for ax, key in zip(axes, keys):
        ...
        ax.set_title(f'{st["label"]}\nMAE={results[key]["mse"]:.5f}', fontsize=7)
...
def plot_so_summary(results: dict, save_path=None):
    ...
    fig.suptitle(_full_title(results, 'Riepilogo Ordine Stocastico (MAE)'),
