"""
main_l1_conBern_pytorch_penalty.py - self-contained
...
OUT = os.path.join(cartella_script, 'immagini_l1_conBern_pytorch_penalty')
...
# --------------------------------------------------------
# METODI DI OTTIMIZZAZIONE - L1  (da methods.py, con MAE)
# --------------------------------------------------------
...
def solve_scipy_l1(N: int, x: np.ndarray, f: np.ndarray,
    ...
    """
    Minimizza MAE(W_ref + Delta) con SLSQP.
    Gradiente L1: grad_Delta = -(M.T @ sign(r)) / n_pts  con r = f - M(W_ref+Delta).
    """
    M = basis_matrix(N, x)
    n_pts = len(x)
    ...
    def jac_obj(Delta):
        r = f - M @ (W_ref + Delta)
        return -(M.T @ np.sign(r)) / n_pts   # grad L1
...
def solve_pytorch_l1(N: int, x: np.ndarray, f: np.ndarray,
    ...
    # Gradient descent Adam con L1Loss, nessun vincolo SO
...
def solve_pytorch_ordered_l1(N: int, x: np.ndarray, f: np.ndarray,
                              ...
                              lam: float = 200.0):
    """
    Gradient descent con penalità SO e L1Loss:
        L(W) = MAE(W) + λ * Σ_h max(0, cumsum(Delta)[h] * segno)²
    """
    ...
    # inizializza dai pesi L1 non vincolati
...
# --------------------------------------------
# RUNNER (adattato per L1 e con metrica MAE)
# --------------------------------------------
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    """
    Esegue tutti i metodi con L1 loss e registra W, Delta, MAE, tempo.
    ...
    def _store(label, W, elapsed, Delta=None):
        ...
        # 'mse' key mantenuto per compatibilità col plotting (contiene MAE)
        return m
...
def plot_so_single(results: dict, key: str, direction: str, save_path=None):
    ...
    # Si guarda se la condizione è soddisfatta
    is_satisfied = so.get('satisfied', False)
    sat_str = '✓ Soddisfatto' if is_satisfied else '✗ Violato'
    
    # Creazione di una variabile solo per il simbolo da usare nella legenda
    ...
    for ax, xd, ct, ca, title in [
        ...
        # Viene inserito dinamicamente il tick_sym nelle label usando le f-string
        if direction == 'upper':
