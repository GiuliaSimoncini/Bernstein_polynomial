"""
main_l1_senzaBern_pytorch_penalty.py - self-contained

Ottimizzazione con distanza L1 (MAE) invece di MSE.
...
Per ciascun esperimento vengono prodotti:
  expN_pdf.png         : PDF confronto + barplot MAE
...
OUT = os.path.join(cartella_script, 'immagini_l1_senzaBern_pytorch_penalty')
...
def solve_scipy_l1(N: int, x: np.ndarray, f: np.ndarray,
...
class _BernsteinModelL1(nn.Module):
...
def solve_pytorch_l1(N: int, x: np.ndarray, f: np.ndarray,
...
def solve_pytorch_ordered_l1(N: int, x: np.ndarray, f: np.ndarray,
                              W_ref: np.ndarray,
                              direction: str = 'upper',
                              epochs: int = 5000,
                              lr: float = 0.02,
    ...
    for _ in range(epochs):
        ...
        mae_term = torch.mean(torch.abs(f_t - M_t @ W))
        pen = lam * stochastic_penalty_torch(W, Wr_t, direction)
        (mae_term + pen).backward()
        opt.step()
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    # BernOp + SO upper
    t0 = time.perf_counter()
    W_bo_up, D_bo_up = solve_scipy_l1(N, x, f,
                                      W_init=W_init.copy(),
                                      direction='upper',
                                      W_ref=W_init.copy())
    ...
    # BernOp + SO lower
    t0 = time.perf_counter()
    W_bo_lo, D_bo_lo = solve_scipy_l1(N, x, f,
                                      W_init=W_init.copy(),
                                      direction='lower',
                                      W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    # Scipy L1 unconstrained
    t0 = time.perf_counter()
    W_scipy, _ = solve_scipy_l1(N, x, f, W_init=W_init.copy())
    t_scipy = (time.perf_counter() - t0) * 1e3
    m = _store('scipy', W_scipy, t_scipy)
    print(f"  Scipy L1             MAE={m:.7f}   t={t_scipy:7.1f} ms")

    # Scipy + SO upper
    t0 = time.perf_counter()
    W_up, D_up = solve_scipy_l1(N, x, f,
                                 W_init=W_scipy.copy(),
                                 direction='upper',
                                 W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    # Scipy + SO lower
    t0 = time.perf_counter()
    W_lo, D_lo = solve_scipy_l1(N, x, f,
                                 W_init=W_scipy.copy(),
                                 direction='lower',
                                 W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    # PyTorch L1 unconstrained
    t0 = time.perf_counter()
    W_pt = solve_pytorch_l1(N, x, f, epochs=epochs_pt)
    t_pytorch = (time.perf_counter() - t0) * 1e3
    m = _store('pytorch', W_pt, t_pytorch)
    print(f"  PyTorch L1           MAE={m:.7f}   t={t_pytorch:7.1f} ms")

    # PyTorch + SO upper
    t0 = time.perf_counter()
    W_pt_up, D_pt_up = solve_pytorch_ordered_l1(N, x, f, W_init.copy(),
                                                 direction='upper')
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (pen.)    MAE={m:.7f}   t={t:7.1f} ms  SO:{sat}")
...
if __name__ == '__main__':
    ...
    # -----------------------
    # GRUPPO A: TARGET BETA
    # -----------------------

    # --------------------------------------------
    # Esp 1 - Beta grado 65 -> Approssimante N=32
    # --------------------------------------------
    print("\n Esperimento 1: Target Beta(33,34) (grado 65) → N=32")
    # Grado: (33-1) + (34-1) = 32 + 33 = 65
    f1 = stats.beta(33, 34).pdf(x)
    r1 = run_experiment(
        name='Target Beta (grado 65)',
        N=32, x=x, f=f1, epochs_pt=3000,
        target_label='PDF Beta(33,34) esatta (grado 65) → approssimato a N=32'
    )
    ...
    # ---------------------------------------------
    print("\n Esperimento 2: Target Beta(17,18) (grado 33) → N=16")
    # Grado: (17-1) + (18-1) = 16 + 17 = 33
    f2 = stats.beta(17, 18).pdf(x)
    r2 = run_experiment(
        name='Target Beta (grado 33)',
        N=16, x=x, f=f2, epochs_pt=3000,
        target_label='PDF Beta(17,18) esatta (grado 33) → approssimato a N=16'
    )
    ...
    # -------------------------------------------
    print("\n Esperimento 3: Target Beta(9,10) (grado 17) → N=8")
    # Grado: (9-1) + (10-1) = 8 + 9 = 17
    f3 = stats.beta(9, 10).pdf(x)
    r3 = run_experiment(
        name='Target Beta (grado 17)',
        N=8, x=x, f=f3, epochs_pt=3000,
        target_label='PDF Beta(9,10) esatta (grado 17) → approssimato a N=8'
    )
    ...
    f4 = (x ** 33) * ((1 - x) ** 32)
    f4 = f4 / np.trapezoid(f4, x) # Normalizzazione
    r4 = run_experiment(
        name='Polinomio puro (grado 65)',
        N=32, x=x, f=f4, epochs_pt=3000,
        target_label='Polinomio x³³(1-x)³² esatto (grado 65) → N=32'
    )
    ...
    f5 = (x ** 17) * ((1 - x) ** 16)
    f5 = f5 / np.trapezoid(f5, x)
    r5 = run_experiment(
        name='Polinomio puro (grado 33)',
        N=16, x=x, f=f5, epochs_pt=3000,
        target_label='Polinomio x¹⁷(1-x)¹⁶ esatto (grado 33) → N=16'
    )
    ...
    f6 = (x ** 9) * ((1 - x) ** 8)
    f6 = f6 / np.trapezoid(f6, x)
    r6 = run_experiment(
        name='Polinomio puro (grado 17)',
        N=8, x=x, f=f6, epochs_pt=3000,
        target_label='Polinomio x⁹(1-x)⁸ esatto (grado 17) → N=8'
    )
    ...
    # ------------------------------------------------
    print("\n Esperimento 7: Beta mixture bimodale (grado 65) → N=32")
    # Grado: (15-1) + (52-1) = 65
    f7 = 0.5 * stats.beta(15, 52).pdf(x) + 0.5 * stats.beta(52, 15).pdf(x)
    f7 = f7 / np.trapezoid(f7, x)
    r7 = run_experiment(
        name='Beta Mixture bimodale (grado 65)',
        N=32, x=x, f=f7, epochs_pt=4000,
        target_label='Mix 0.5·Beta(15,52) + 0.5·Beta(52,15) → N=32'
    )
    ...
    # ------------------------------------------------
    print("\n Esperimento 8: Beta mixture bimodale (grado 33) → N=16")
    # Grado: (8-1) + (27-1) = 33
    f8 = 0.5 * stats.beta(8, 27).pdf(x) + 0.5 * stats.beta(27, 8).pdf(x)
    f8 = f8 / np.trapezoid(f8, x)
    r8 = run_experiment(
        name='Beta Mixture bimodale (grado 33)',
        N=16, x=x, f=f8, epochs_pt=4000,
        target_label='Mix 0.5·Beta(8,27) + 0.5·Beta(27,8) → N=16'
    )
    ...
    # -----------------------------------------------
    print("\n Esperimento 9: Beta mixture bimodale (grado 17) → N=8")
    # Grado: (4-1) + (15-1) = 17
    f9 = 0.5 * stats.beta(4, 15).pdf(x) + 0.5 * stats.beta(15, 4).pdf(x)
    f9 = f9 / np.trapezoid(f9, x)
    r9 = run_experiment(
        name='Beta Mixture bimodale (grado 17)',
        N=8, x=x, f=f9, epochs_pt=4000,
        target_label='Mix 0.5·Beta(4,15) + 0.5·Beta(15,4) → N=8'
    )
