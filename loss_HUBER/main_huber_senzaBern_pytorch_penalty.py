...
La Huber loss combina MSE (vicino allo zero) e MAE (per grandi residui).
...
OUT = os.path.join(cartella_script, 'immagini_huber_senzaBern_pytorch_penalty')
os.makedirs(OUT, exist_ok=True)

# Parametro di transizione per la Huber Loss
DELTA_HUB = 1e-3 
...
# ------------------------------------------
# METODI DI OTTIMIZZAZIONE - con HUBER loss
# ------------------------------------------
...
def solve_scipy_huber(N: int, x: np.ndarray, f: np.ndarray,
                      W_init=None, direction=None, W_ref=None, delta: float = DELTA_HUB):
    M = basis_matrix(N, x)
    n_pts = len(x)
    ...
    def jac_obj(Delta):
        r = f - M @ (W_ref + Delta)
        grad_r = np.where(np.abs(r) <= delta, r, delta * np.sign(r))
...
def solve_pytorch_huber(N: int, x: np.ndarray, f: np.ndarray,
                        epochs: int = 3000, lr: float = 0.05, delta: float = DELTA_HUB) -> np.ndarray:
    ...
    loss_fn = nn.HuberLoss(reduction='mean', delta=delta)
...
def solve_pytorch_ordered_huber(N: int, x: np.ndarray, f: np.ndarray,
    ...
    W_init = solve_pytorch_huber(N, x, f, delta=delta)
    ...
    for _ in range(epochs):
        ...
        huber_term = loss_fn(M_t @ W, f_t)
        pen = lam * stochastic_penalty_torch(W, Wr_t, direction)
        (huber_term + pen).backward()
        opt.step()
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    def _store(label, W, elapsed, Delta=None):
        loss_val = huber_loss(W, M, f)
        results[label] = dict(W=W, mse=loss_val, time_ms=elapsed, Delta=Delta)
        return loss_val

    # Bernstein operator
    m = _store('bernstein_op', W_init.copy(), t_bern)
    print(f"  Bernstein operator   Loss={m:.7f}   t={t_bern:7.1f} ms")
    ...
    print(f"  BernOp+SO upper      Loss={m:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  BernOp+SO lower      Loss={m:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    print(f"  Ing-Cane             Loss={m:.7f}   t={t:7.1f} ms")
    ...
    print(f"  Scipy Huber          Loss={m:.7f}   t={t_scipy:7.1f} ms")
    ...
    print(f"  Scipy+SO upper       Loss={m:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    print(f"  Scipy+SO lower       Loss={m:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    print(f"  PyTorch Huber        Loss={m:.7f}   t={t_pytorch:7.1f} ms")
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (pen.)    Loss={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # Riepilogo
    print(f"\n  {'Metodo':<28}  {'Huber Loss':>12}  {'Tempo (ms)':>12}  SO")
    print(f"  {'-'*58}")
...
STYLE = {
    ...
    'pytorch_upper':      dict(color='#c0392b', ls='--', lw=1.6, label='PyTorch Huber + SO (pen.)'),
}
...
def plot_weights(results: dict, save_path=None):
    ...
    for ax, key in zip(axes, keys):
        ...
        ax.set_title(f'{st["label"]}\nLoss={results[key]["mse"]:.5f}', fontsize=7)
...
def plot_so_summary(results: dict, save_path=None):
    ...
    fig.suptitle(_full_title(results, 'Riepilogo Ordine Stocastico (Huber Loss)'),
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*140}")
    print(f"  Tutti i 12 esperimenti (funzioni target pure con Huber Loss) completati.")
