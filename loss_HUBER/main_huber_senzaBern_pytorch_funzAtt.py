"""
main_huber_senzaBern_pytorch_funzAtt.py - self-contained
...
OUT = os.path.join(cartella_script, 'immagini_huber_senzaBern_pytorch_funzAtt')
os.makedirs(OUT, exist_ok=True)

# Parametro delta della Huber loss: soglia tra regime quadratico e lineare.
# Valori tipici: 0.1 (più robusta) ... 1.0 (più simile a MSE).
DELTA_HUB = 0.5
...
def huber_loss(W: np.ndarray, M: np.ndarray, f: np.ndarray, delta: float = DELTA_HUB) -> float:
...
def solve_scipy_huber(N: int, x: np.ndarray, f: np.ndarray,
    ...
    def objective(Delta):
        r = f - M @ (W_ref + Delta)
        abs_r = np.abs(r)
        loss = np.where(abs_r <= DELTA_HUB,
                        0.5 * r ** 2,
                        DELTA_HUB * (abs_r - 0.5 * DELTA_HUB))
        return float(np.mean(loss))

    def jac_obj(Delta):
        r = f - M @ (W_ref + Delta)
        # Calcolo del gradiente per la Huber Loss
        grad_r = np.where(np.abs(r) <= DELTA_HUB, r, DELTA_HUB * np.sign(r))
        return -(M.T @ grad_r) / n_pts
...
class _StochasticOrderedModelHuber(nn.Module):
    """
...
def solve_pytorch_huber(N: int, x: np.ndarray, f: np.ndarray,
    ...
    model = _BernsteinModelHuber(N)
    opt = optim.Adam(model.parameters(), lr=lr)
    loss_fn = nn.HuberLoss(delta=DELTA_HUB)
...
def solve_pytorch_ordered_huber(N: int, x: np.ndarray, f: np.ndarray,
                              W_ref: np.ndarray,
                              direction: str = 'upper',
                              epochs: int = 4000,
                              lr: float = 0.05):
    ...
    model = _StochasticOrderedModelHuber(N, W_ref, direction=direction)
    ...
    for _ in range(epochs):
        opt.zero_grad()
        # Calcola solo Huber loss. I vincoli sono matematicamente garantiti dal forward pass.
        huber_term = loss_fn(model(M_t), f_t)
        huber_term.backward()
        opt.step()
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    def _store(label, W, elapsed, Delta=None):
        h_val = huber_loss(W, M, f)
        results[label] = dict(W=W, mse=h_val, time_ms=elapsed, Delta=Delta)
        return h_val

    # Bernstein operator
    m = _store('bernstein_op', W_init.copy(), t_bern)
    print(f"  Bernstein operator   Huber={m:.7f}   t={t_bern:7.1f} ms")

    # BernOp + SO upper
    t0 = time.perf_counter()
    W_bo_up, D_bo_up = solve_scipy_huber(N, x, f,
                                      W_init=W_init.copy(),
                                      direction='upper',
                                      W_ref=W_init.copy())
    ...
    print(f"  BernOp+SO upper      Huber={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # BernOp + SO lower
    t0 = time.perf_counter()
    W_bo_lo, D_bo_lo = solve_scipy_huber(N, x, f,
                                      W_init=W_init.copy(),
                                      direction='lower',
                                      W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  BernOp+SO lower      Huber={m:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    print(f"  Ing-Cane             Huber={m:.7f}   t={t:7.1f} ms")

    # Scipy Huber unconstrained
    t0 = time.perf_counter()
    W_scipy, _ = solve_scipy_huber(N, x, f, W_init=W_init.copy())
    t_scipy = (time.perf_counter() - t0) * 1e3
    m = _store('scipy', W_scipy, t_scipy)
    print(f"  Scipy Huber            Huber={m:.7f}   t={t_scipy:7.1f} ms")

    # Scipy + SO upper
    t0 = time.perf_counter()
    W_up, D_up = solve_scipy_huber(N, x, f,
                                 W_init=W_scipy.copy(),
                                 direction='upper',
                                 W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    print(f"  Scipy+SO upper       Huber={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # Scipy + SO lower
    t0 = time.perf_counter()
    W_lo, D_lo = solve_scipy_huber(N, x, f,
                                 W_init=W_scipy.copy(),
                                 direction='lower',
                                 W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    print(f"  Scipy+SO lower       Huber={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # PyTorch Huber unconstrained
    t0 = time.perf_counter()
    W_pt = solve_pytorch_huber(N, x, f, epochs=epochs_pt)
    t_pytorch = (time.perf_counter() - t0) * 1e3
    m = _store('pytorch', W_pt, t_pytorch)
    print(f"  PyTorch Huber          Huber={m:.7f}   t={t_pytorch:7.1f} ms")
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (Attiv.) upp  Huber={m:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (Attiv.) low  Huber={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # Riepilogo
    print(f"\n  {'Metodo':<28}  {'Huber':>12}  {'Tempo (ms)':>12}  SO")
    print(f"  {'-'*58}")
...
STYLE = {
    ...
    'scipy':              dict(color='#27ae60', ls='-',  lw=2.0, label='Scipy Huber SLSQP'),
    'scipy_upper':        dict(color='#e67e22', ls='-',  lw=1.8, label='Scipy Huber + SO upper'),
    'scipy_lower':        dict(color='#8e44ad', ls='-',  lw=1.8, label='Scipy Huber + SO lower'),
    'pytorch':            dict(color='#2980b9', ls='--', lw=1.8, label='PyTorch Huber'),
    'pytorch_upper':      dict(color='#c0392b', ls='--', lw=1.6, label='PyTorch Huber + SO upp (Attivaz.)'),
    'pytorch_lower':      dict(color="#c02b93", ls='--', lw=1.6, label='PyTorch Huber + SO low (Attivaz.)')
}
...
def plot_weights(results: dict, save_path=None):
    ...
    for ax, key in zip(axes, keys):
        ...
        ax.set_title(f'{st["label"]}\nHuber={results[key]["mse"]:.5f}', fontsize=7)
...
def plot_so_summary(results: dict, save_path=None):
    ...
    fig.suptitle(_full_title(results, 'Riepilogo Ordine Stocastico (Huber)'),
