...
OUT = os.path.join(cartella_script, 'immagini_huber_conBern_pytorch_penalty')
...
def _huber_grad(r: np.ndarray, delta: float) -> np.ndarray:
    """
    Gradiente della Huber loss rispetto ai residui r = f - M@W:
        dL/dr = r             se |r| <= delta
              = delta*sign(r) altrimenti
    Utile per il calcolo del gradiente analitico in SLSQP.
    """
...
def solve_scipy_huber(N: int, x: np.ndarray, f: np.ndarray,
    ...
    """
    Minimizza Huber(W_ref + Delta) con SLSQP.
    Gradiente Huber: grad_Delta = -(M.T @ huber_grad(r)) / n_pts
      con r = f - M@(W_ref + Delta).
    """
    M = basis_matrix(N, x)
    n_pts = len(x)
    ...
    def jac_obj(Delta):
        r = f - M @ (W_ref + Delta)
        # grad rispetto a W: chain rule attraverso r = f - M@W
        return -(M.T @ _huber_grad(r, delta)) / n_pts
...
def solve_pytorch_huber(N: int, x: np.ndarray, f: np.ndarray,
                        ...
                        delta: float = DELTA_HUB) -> np.ndarray:
    """
    Gradient descent Adam con Huber loss (HuberLoss di PyTorch), nessun vincolo SO.
    Il parametro delta di HuberLoss corrisponde alla soglia quadratico/lineare.
    """
    ...
    # nn.HuberLoss usa reduction='mean' di default; delta=DELTA_HUB
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    """
    Esegue tutti i metodi con Huber loss e registra W, Delta, Huber loss, tempo.
    ...
    # Bernstein operator
    h = _store('bernstein_op', W_init.copy(), t_bern)
    print(f"  Bernstein operator   Huber={h:.7f}   t={t_bern:7.1f} ms")
    ...
    h = _store('bernstein_op_upper', W_bo_up, t, Delta=D_bo_up)
    results['bernstein_op_upper']['so']    = so
    results['bernstein_op_upper']['W_ref'] = W_init.copy()
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  BernOp+SO upper      Huber={h:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    so = check_order(cdf_a, cdf_t, 'upper')
    h = _store('bernstein_op_lower', W_bo_lo, t, Delta=D_bo_lo)
    results['bernstein_op_lower']['so']    = so
    results['bernstein_op_lower']['W_ref'] = W_init.copy()
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  BernOp+SO lower      Huber={h:.7f}   t={t:7.1f} ms  SO:{sat}")

    # Ing-Cane
    t0 = time.perf_counter()
    W_ic = solve_ing_cane(N, x, f)
    t = (time.perf_counter() - t0) * 1e3
    h = _store('ing_cane', W_ic, t)
    print(f"  Ing-Cane             Huber={h:.7f}   t={t:7.1f} ms")

    # Scipy Huber unconstrained
    t0 = time.perf_counter()
    W_scipy, _ = solve_scipy_huber(N, x, f, W_init=W_init.copy())
    t_scipy = (time.perf_counter() - t0) * 1e3
    h = _store('scipy', W_scipy, t_scipy)
    print(f"  Scipy Huber          Huber={h:.7f}   t={t_scipy:7.1f} ms")
    ...
    so = check_order(cdf_a, cdf_t, 'upper')
    h = _store('pytorch_upper', W_pt_up, t, Delta=D_pt_up)
    results['pytorch_upper']['so']    = so
    results['pytorch_upper']['W_ref'] = W_init.copy()
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (pen.)    Huber={h:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    for k in METHOD_ORDER:
        ...
        print(f"  {k:<28}  {hv:12.7f}  {tv:12.1f}{sat}")
...
def plot_pdf_comparison(results: dict, save_path=None):
    ...
    fig, (ax_pdf, ax_hub) = plt.subplots(1, 2, figsize=(14, 5))
    ...
    hubs  = [results[k]['mse']      for k in keys]
    times = [results[k]['time_ms']  for k in keys]
    cols  = [STYLE[k]['color']      for k in keys]
    lbls  = [STYLE[k]['label']      for k in keys]

    bars = ax_hub.bar(range(len(keys)), hubs, color=cols, alpha=0.8)
    ax_hub.set_xticks(range(len(keys)))
    ax_hub.set_xticklabels(lbls, rotation=40, ha='right', fontsize=7)
    ax_hub.set_ylabel('Huber loss'); ax_hub.set_title('Huber loss per metodo')
    ax_hub.grid(True, axis='y', alpha=0.3)
    for bar, t in zip(bars, times):
        h = bar.get_height()
        label = f'{t:.0f}ms' if t > 0.5 else '<1ms'
        ax_hub.text(bar.get_x() + bar.get_width() / 2, h * 1.04,
