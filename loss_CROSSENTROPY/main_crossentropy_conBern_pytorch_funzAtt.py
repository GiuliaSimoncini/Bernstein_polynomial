"""
main_crossentropy_conBern_pytorch_funzAtt.py - self-contained

Ottimizzazione con distanza Cross Entropy Loss.
...
OUT = os.path.join(cartella_script, 'immagini_crossentropy_conBern_pytorch_funzAtt')
...
def solve_scipy_ce(N: int, x: np.ndarray, f: np.ndarray,
    ...
    def objective(Delta):
        # Usiamo direttamente la funzione CE precedentemente calcolata
...
def pytorch_ce_loss(output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    # Calcola la Cross Entropy loss per PyTorch
...
def solve_pytorch_ordered_ce(N: int, x: np.ndarray, f: np.ndarray,
    ...
    for _ in range(epochs):
        opt.zero_grad()
        # Calcola solo il CE. I vincoli sono matematicamente garantiti dal forward pass.
        ce_term = loss_fn(model(M_t), f_t)
        ce_term.backward()
        opt.step()
