"""
main_crossentropy_senzaBern_pytorch_penalty.py - self-contained

Ottimizzazione con distanza CE.
- Target: Funzioni analitiche esatte valutate su x (senza mappare su BP).
- Grado Target: 2N+1
- Grado Approssimante: N
- Controllo Ordine Stocastico: Eseguito rigorosamente sulle vere CDF.
...
OUT = os.path.join(cartella_script, 'immagini_crossentropy_senzaBern_pytorch_penalty')
...
def solve_scipy_ce(N: int, x: np.ndarray, f: np.ndarray,
    ...
    # Usa approssimazione numerica del gradiente (più stabile per CE manuale)
...
def solve_pytorch_ordered_ce(N: int, x: np.ndarray, f: np.ndarray,
    ...
    for _ in range(epochs):
        ...
        ce_term = ce_loss_torch(M_t @ W, f_t)
        pen = lam * stochastic_penalty_torch(W, Wr_t, direction)
        
        # Somma la Cross Entropy alla penalità
        (ce_term + pen).backward() 
        opt.step()
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    def _store(label, W, elapsed, Delta=None):
        val = ce_loss(W, M, f)
        results[label] = dict(W=W, mse=val, time_ms=elapsed, Delta=Delta)
        return val
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (pen.)    CE={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # Riepilogo
    print(f"\n  {'Metodo':<28}  {'CE Loss':>12}  {'Tempo (ms)':>12}  SO")
    print(f"  {'-'*58}")
...
STYLE = {
    ...
    'pytorch_upper':      dict(color='#c0392b', ls='--', lw=1.6, label='PyTorch CE + SO (pen.)'),
}
...
def plot_pdf_comparison(results: dict, save_path=None):
    ...
    ax_mse.set_ylabel('CE'); ax_mse.set_title('Cross Entropy Loss')
...
def plot_weights(results: dict, save_path=None):
    ...
    for ax, key in zip(axes, keys):
        ...
        ax.set_title(f'{st["label"]}\nCE={results[key]["mse"]:.5f}', fontsize=7)
...
if __name__ == '__main__':
    ...
    # Supporto [0,2] scalato su [0,1] per ottenere la PDF target finale
    ...
    print(f"\n{'='*140}")
    print(f"  Tutti i 12 esperimenti (funzioni target PURE valutate con CE) completati.")
