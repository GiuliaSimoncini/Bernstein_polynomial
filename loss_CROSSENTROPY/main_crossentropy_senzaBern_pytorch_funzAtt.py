"""
main_jesen_crossentropy_pytorch_funzAtt.py - self-contained

Ottimizzazione con distanza CE.
...
OUT = os.path.join(cartella_script, 'immagini_crossentropy_senzaBern_pytorch_funzAtt')
...
def ce_loss(W: np.ndarray, M: np.ndarray, f: np.ndarray) -> float:
    # Calcola la Cross Entropy tra la PDF target f e l'approssimazione M@W.
...
def solve_scipy_ce(N: int, x: np.ndarray, f: np.ndarray,
...
class _BernsteinModelCE(nn.Module):
...
class _StochasticOrderedModelCE(nn.Module):
    """
...
def ce_loss_torch(pred_pdf: torch.Tensor, target_pdf: torch.Tensor) -> torch.Tensor:
    # Implementazione PyTorch differenziabile della Cross Entropy
    P = torch.clamp(target_pdf, min=1e-12)
    P = P / torch.sum(P)
    
    Q = torch.clamp(pred_pdf, min=1e-12)
    Q = Q / torch.sum(Q)
    
    return -torch.sum(P * torch.log(Q))


def solve_pytorch_ce(N: int, x: np.ndarray, f: np.ndarray,
    ...
    model = _BernsteinModelCE(N)
    opt = optim.Adam(model.parameters(), lr=lr)
    loss_fn = ce_loss_torch

    for _ in range(epochs):
        opt.zero_grad()
        loss = ce_loss_torch(model(M_t), f_t)
        loss.backward()
        opt.step()

    return model.weights()

def solve_pytorch_ordered_ce(N: int, x: np.ndarray, f: np.ndarray,
                              W_ref: np.ndarray,
                              direction: str = 'upper',
                              epochs: int = 4000,
                              lr: float = 0.05):
    ...
    model = _StochasticOrderedModelCE(N, W_ref, direction=direction)
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    def _store(label, W, elapsed, Delta=None):
        val = ce_loss(W, M, f)
        results[label] = dict(W=W, ce=val, time_ms=elapsed, Delta=Delta)
        return val

    # Bernstein operator
    m = _store('bernstein_op', W_init.copy(), t_bern)
    print(f"  Bernstein operator   CE={m:.7f}   t={t_bern:7.1f} ms")

    # BernOp + SO upper
    t0 = time.perf_counter()
    W_bo_up, D_bo_up = solve_scipy_ce(N, x, f,
                                      W_init=W_init.copy(),
                                      direction='upper',
                                      W_ref=W_init.copy())
    ...
    print(f"  BernOp+SO upper      CE={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # BernOp + SO lower
    t0 = time.perf_counter()
    W_bo_lo, D_bo_lo = solve_scipy_ce(N, x, f,
                                      W_init=W_init.copy(),
                                      direction='lower',
                                      W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  BernOp+SO lower      CE={m:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    print(f"  Ing-Cane             CE={m:.7f}   t={t:7.1f} ms")

    # Scipy CE unconstrained
    t0 = time.perf_counter()
    W_scipy, _ = solve_scipy_ce(N, x, f, W_init=W_init.copy())
    t_scipy = (time.perf_counter() - t0) * 1e3
    m = _store('scipy', W_scipy, t_scipy)
    print(f"  Scipy CE            CE={m:.7f}   t={t_scipy:7.1f} ms")

    # Scipy + SO upper
    t0 = time.perf_counter()
    W_up, D_up = solve_scipy_ce(N, x, f,
                                 W_init=W_scipy.copy(),
                                 direction='upper',
                                 W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    print(f"  Scipy+SO upper       CE={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # Scipy + SO lower
    t0 = time.perf_counter()
    W_lo, D_lo = solve_scipy_ce(N, x, f,
                                 W_init=W_scipy.copy(),
                                 direction='lower',
                                 W_ref=W_init.copy())
    t = (time.perf_counter() - t0) * 1e3
    ...
    print(f"  Scipy+SO lower       CE={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # PyTorch CE unconstrained
    t0 = time.perf_counter()
    W_pt = solve_pytorch_ce(N, x, f, epochs=epochs_pt)
    t_pytorch = (time.perf_counter() - t0) * 1e3
    m = _store('pytorch', W_pt, t_pytorch)
    print(f"  PyTorch CE          CE={m:.7f}   t={t_pytorch:7.1f} ms")

    # PyTorch + SO upper (Nuova Attivazione)
    t0 = time.perf_counter()
    W_pt_up, D_pt_up = solve_pytorch_ordered_ce(N, x, f, W_init.copy(),
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (Attiv.) upp  CE={m:.7f}   t={t:7.1f} ms  SO:{sat}")
    ...
    sat = '✓' if so['satisfied'] else '✗'
    print(f"  PyTorch+SO (Attiv.) low  CE={m:.7f}   t={t:7.1f} ms  SO:{sat}")

    # Riepilogo
    print(f"\n  {'Metodo':<28}  {'CE':>12}  {'Tempo (ms)':>12}  SO")
    print(f"  {'-'*58}")
...
STYLE = {
    ...
    'scipy':              dict(color='#27ae60', ls='-',  lw=2.0, label='Scipy CE SLSQP'),
    'scipy_upper':        dict(color='#e67e22', ls='-',  lw=1.8, label='Scipy CE + SO upper'),
    'scipy_lower':        dict(color='#8e44ad', ls='-',  lw=1.8, label='Scipy CE + SO lower'),
    'pytorch':            dict(color='#2980b9', ls='--', lw=1.8, label='PyTorch CE'),
    'pytorch_upper':      dict(color='#c0392b', ls='--', lw=1.6, label='PyTorch CE + SO upp (Attivaz.)'),
    'pytorch_lower':      dict(color="#c02b93", ls='--', lw=1.6, label='PyTorch CE + SO low (Attivaz.)')
}
...
def plot_pdf_comparison(results: dict, save_path=None):
    ...
    fig, (ax_pdf, ax_ce) = plt.subplots(1, 2, figsize=(14, 5))
    ...
    ces  = [results[k]['ce']      for k in keys]
    times = [results[k]['time_ms']  for k in keys]
    cols  = [STYLE[k]['color']      for k in keys]
    lbls  = [STYLE[k]['label']      for k in keys]

    bars = ax_ce.bar(range(len(keys)), ces, color=cols, alpha=0.8)
    ax_ce.set_xticks(range(len(keys)))
    ax_ce.set_xticklabels(lbls, rotation=40, ha='right', fontsize=7)
    ax_ce.set_ylabel('CE'); ax_ce.set_title('CE per metodo')
    ax_ce.grid(True, axis='y', alpha=0.3)
    for bar, t in zip(bars, times):
        h = bar.get_height()
        label = f'{t:.0f}ms' if t > 0.5 else '<1ms'
        ax_ce.text(bar.get_x() + bar.get_width() / 2, h * 1.04,
...
def plot_weights(results: dict, save_path=None):
    ...
    for ax, key in zip(axes, keys):
        ...
        ax.set_title(f'{st["label"]}\nCE={results[key]["ce"]:.5f}', fontsize=7)
...
def plot_so_summary(results: dict, save_path=None):
    ...
    fig.suptitle(_full_title(results, 'Riepilogo Ordine Stocastico (CE)'),
...
if __name__ == '__main__':
    ...
    # ----------------------------------------------
    # GRUPPO A: TARGET BETA PURE (grado 2N+1 -> N)
    # ----------------------------------------------

    # ---------------------------------------------
    # Esp 1 - Beta grado 65 -> Approssimante N=32
    # ---------------------------------------------
    print("\n Esperimento 1: Target Beta(33,34) pura (grado 65) → N=32")
    f1 = stats.beta(33, 34).pdf(x)
    r1 = run_experiment(
        name='Target Beta (grado 65)',
        N=32, x=x, f=f1, epochs_pt=3000,
        target_label='Beta(33,34) pura → N=32'
    )
    ...
    # ----------------------------------------------
    print("\n Esperimento 2: Target Beta(17,18) pura (grado 33) → N=16")
    f2 = stats.beta(17, 18).pdf(x)
    r2 = run_experiment(
        name='Target Beta (grado 33)',
        N=16, x=x, f=f2, epochs_pt=3000,
        target_label='Beta(17,18) pura → N=16'
    )
    ...
    # --------------------------------------------
    print("\n Esperimento 3: Target Beta(9,10) pura (grado 17) → N=8")
    f3 = stats.beta(9, 10).pdf(x)
    r3 = run_experiment(
        name='Target Beta (grado 17)',
        N=8, x=x, f=f3, epochs_pt=3000,
        target_label='Beta(9,10) pura → N=8'
    )
    save_all_plots(r3, 'exp3')


    # --------------------------------
    # GRUPPO B: TARGET POLINOMIO PURO
    # --------------------------------

    # --------------------------------------------------
    # Esp 4 - Polinomio grado 65 -> Approssimante N=32
    # --------------------------------------------------
    print("\n Esperimento 4: Target Polinomio x³³(1-x)³² (grado 65) → N=32")
    f4_raw = (x ** 33) * ((1 - x) ** 32)
    f4 = f4_raw / np.trapezoid(f4_raw, x)
    r4 = run_experiment(
        name='Polinomio puro (grado 65)',
        N=32, x=x, f=f4, epochs_pt=3000,
        target_label='Polinomio x³³(1-x)³² puro → N=32'
    )
    ...
    # --------------------------------------------------
    print("\n Esperimento 5: Target Polinomio x¹⁷(1-x)¹⁶ (grado 33) → N=16")
    f5_raw = (x ** 17) * ((1 - x) ** 16)
    f5 = f5_raw / np.trapezoid(f5_raw, x)
    r5 = run_experiment(
        name='Polinomio puro (grado 33)',
        N=16, x=x, f=f5, epochs_pt=3000,
        target_label='Polinomio x¹⁷(1-x)¹⁶ puro → N=16'
    )
    ...
    # -------------------------------------------------
    print("\n Esperimento 6: Target Polinomio x⁹(1-x)⁸ (grado 17) → N=8")
    f6_raw = (x ** 9) * ((1 - x) ** 8)
    f6 = f6_raw / np.trapezoid(f6_raw, x)
    r6 = run_experiment(
        name='Polinomio puro (grado 17)',
        N=8, x=x, f=f6, epochs_pt=3000,
        target_label='Polinomio x⁹(1-x)⁸ puro → N=8'
    )
    save_all_plots(r6, 'exp6')


    # ----------------------------------
    # GRUPPO C: TARGET MIXTURE BIMODALE
    # ----------------------------------

    # -----------------------------------------------
    # Esp 7 - Mixture grado 65 -> Approssimante N=32
    # -----------------------------------------------
    print("\n Esperimento 7: Target Beta mixture bimodale (grado 65) → N=32")
    f7 = 0.5 * stats.beta(15, 52).pdf(x) + 0.5 * stats.beta(52, 15).pdf(x)
    r7 = run_experiment(
        name='Beta Mixture (grado 65)',
        N=32, x=x, f=f7, epochs_pt=4000,
        target_label='Mix 0.5·Beta(15,52)+0.5·Beta(52,15) pura → N=32'
    )
    ...
    # -----------------------------------------------
    print("\n Esperimento 8: Target Beta mixture bimodale (grado 33) → N=16")
    f8 = 0.5 * stats.beta(8, 27).pdf(x) + 0.5 * stats.beta(27, 8).pdf(x)
    r8 = run_experiment(
        name='Beta Mixture (grado 33)',
        N=16, x=x, f=f8, epochs_pt=4000,
        target_label='Mix 0.5·Beta(8,27)+0.5·Beta(27,8) pura → N=16'
    )
    ...
    # ----------------------------------------------
    print("\n Esperimento 9: Target Beta mixture bimodale (grado 17) → N=8")
    f9 = 0.5 * stats.beta(4, 15).pdf(x) + 0.5 * stats.beta(15, 4).pdf(x)
    r9 = run_experiment(
        name='Beta Mixture (grado 17)',
        N=8, x=x, f=f9, epochs_pt=4000,
        target_label='Mix 0.5·Beta(4,15)+0.5·Beta(15,4) pura → N=8'
    )
    save_all_plots(r9, 'exp9')


    # -----------------------------------------
    # GRUPPO D: TARGET CONVOLUZIONE NUMERICA
    # -----------------------------------------

    # -------------------------------------------------------
    # Esp 10 - Convoluzione grado 65 -> Approssimante N=32
    # -------------------------------------------------------
    print("\n Esperimento 10: Convoluzione numerica (grado eq. 65) → N=32")
    f_c10_a = stats.beta(12, 22).pdf(x)
    f_c10_b = stats.beta(22, 13).pdf(x)
    conv10 = np.convolve(f_c10_a, f_c10_b, mode='full')
    f10 = np.interp(x, np.linspace(0, 2, len(conv10)) / 2.0, conv10)
    f10 /= np.trapezoid(f10, x)
    
    r10 = run_experiment(
        name='Convoluzione (grado eq 65)',
        N=32, x=x, f=f10, epochs_pt=4000,
        target_label='Conv pura Beta(12,22)+Beta(22,13) scalata a [0,1] → N=32'
    )
    ...
    # -------------------------------------------------------
    print("\n Esperimento 11: Convoluzione numerica (grado eq. 33) → N=16")
    f_c11_a = stats.beta(5, 13).pdf(x)
    f_c11_b = stats.beta(13, 6).pdf(x)
    conv11 = np.convolve(f_c11_a, f_c11_b, mode='full')
    f11 = np.interp(x, np.linspace(0, 2, len(conv11)) / 2.0, conv11)
    f11 /= np.trapezoid(f11, x)
    
    r11 = run_experiment(
        name='Convoluzione (grado eq 33)',
        N=16, x=x, f=f11, epochs_pt=4000,
        target_label='Conv pura Beta(5,13)+Beta(13,6) scalata a [0,1] → N=16'
    )
    ...
    # -------------------------------------------------------
    print("\n Esperimento 12: Convoluzione numerica (grado eq. 17) → N=8")
    f_c12_a = stats.beta(3, 7).pdf(x)
    f_c12_b = stats.beta(7, 4).pdf(x)
    conv12 = np.convolve(f_c12_a, f_c12_b, mode='full')
    f12 = np.interp(x, np.linspace(0, 2, len(conv12)) / 2.0, conv12)
    f12 /= np.trapezoid(f12, x)
    
    r12 = run_experiment(
        name='Convoluzione (grado eq 17)',
        N=8, x=x, f=f12, epochs_pt=4000,
        target_label='Conv pura Beta(3,7)+Beta(7,4) scalata a [0,1] → N=8'
    )
    save_all_plots(r12, 'exp12')

    print(f"\n{'='*140}")
    print(f"  Tutti i 12 esperimenti (funzioni target PURE) completati.")
