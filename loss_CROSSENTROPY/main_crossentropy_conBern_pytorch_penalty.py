"""
main_crossentropy_conBern_pytorch_penalty.py  -  self-contained

Ottimizzazione con distanza CE invece di MSE.
...
Per ciascun esperimento vengono prodotti:
  expN_pdf.png         : PDF confronto + barplot CE
...
OUT = os.path.join(cartella_script, 'immagini_crossentropy_conBern_pytorch_penalty')
...
# ----------------------------------------------------
# METODI DI OTTIMIZZAZIONE (da methods.py, con CE)
# ----------------------------------------------------
...
def solve_scipy_ce(N: int, x: np.ndarray, f: np.ndarray,
    ...
    # Minimizza la CE(W_ref + Delta) usando SLSQP con gradiente numerico
    M = basis_matrix(N, x)
...
def run_experiment(name: str, N: int, x: np.ndarray, f: np.ndarray,
    ...
    """
    Esegue tutti i metodi con CE loss e registra W, Delta, tempo.
...
def plot_pdf_comparison(results: dict, save_path=None):
    ...
    ax_mse.set_ylabel('CE'); ax_mse.set_title('CE per metodo')
